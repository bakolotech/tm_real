var CryptoJS = CryptoJS || function(b, e) {
    var f = {}, 
        c = f.lib = {},
        h = function() {},
        y = c.Base = {
        ...
        l = c.WordArray = y.extend({
            ...
            random: function(g) {
                for (var p = [], d = 0; d < g; d += 4) p.push(4294967296 * b.random() | 0);
        ...
        u = c.BufferedBlockAlgorithm = y.extend({
            reset: function() {
            ...
            _process: function(g) {
                var p = this._data,
                    ...
                    n = this.blockSize,
                    x = k / (4 * n);
                x = g ? b.ceil(x) : b.max((x | 0) - this._minBufferSize, 0);
                g = x * n;
                k = b.min(4 * g, k);
                if (g) {
...
(function() {
    ...
    b.enc.Base64 = {
        stringify: function(f) {
            ...
            f = [];
            for (var l = 0; l < h; l += 3)
                for (var B = (c[l >>> 2] >>> 24 - l % 4 * 8 & 255) << 16 | (c[l + 1 >>> 2] >>> 24 - (l + 1) % 4 * 8 & 255) << 8 | c[l + 2 >>> 2] >>> 24 - (l + 2) % 4 * 8 & 255, a = 0; 4 > a && l + .75 * a < h; a++) f.push(y.charAt(B >>> 6 * (3 - a) & 63));
            if (c = y.charAt(64))
                for (; f.length % 4;) f.push(c);
            return f.join("")
        },
        parse: function(f) {
            var c = f.length,
...
(function(b) {
    ...
    var y = CryptoJS,
        ...
        a = l.Hasher;
    l = y.algo;
    for (var m = [], z = 0; 64 > z; z++) m[z] = 4294967296 * b.abs(b.sin(z + 1)) | 0;
    l = l.MD5 = a.extend({
        _doReset: function() {
        ...
        _doProcessBlock: function(u, w) {
            ...
            q = c(q, v, r, t, A, 4, m[32]);
            t = c(t, q, v, r, O, 11, m[33]);
            r = c(r, t, q, v, C, 16, m[34]);
            v = c(v, r, t, q, E, 23, m[35]);
            q = c(q, v, r, t, d, 4, m[36]);
            t = c(t, q, v, r, x, 11, m[37]);
            r = c(r, t, q, v, G, 16, m[38]);
            v = c(v, r, t, q, F, 23, m[39]);
            q = c(q, v, r, t, H, 4, m[40]);
            t = c(t, q, v, r, p, 11, m[41]);
            r = c(r, t, q, v, n, 16, m[42]);
            v = c(v, r, t, q, I, 23, m[43]);
            q = c(q, v, r, t, D, 4, m[44]);
            t = c(t, q, v, r, J, 11, m[45]);
            r = c(r, t, q, v, L, 16, m[46]);
            v = c(v, r, t, q, k, 23, m[47]);
...
CryptoJS.lib.Cipher || function(b) {
    ...
    var m = e.mode = {},
        z = function(d, k, n) {
            var x = this._iv;
            x ? this._iv = b : x = this._prevBlock;
            for (var A = 0; A < n; A++) d[k + A] ^=
                x[A]
        },
        u = (f.BlockCipherMode = c.extend({
            createEncryptor: function(d, k) {
    ...
    var g = f.SerializableCipher = c.extend({
        cfg: c.extend({
            format: m
        }),
...
(function() {
    ...
    b.algo.HMAC = b.lib.Base.extend({
        init: function(f, c) {
            ...
            "string" == typeof c && (c = e.parse(c));
            var h = f.blockSize,
                y = 4 * h;
            c.sigBytes > y && (c = f.finalize(c));
            c.clamp();
            for (var l = this._oKey = c.clone(), B = this._iKey = c.clone(), a = l.words, m = B.words, z = 0; z < h; z++) a[z] ^= 1549556828, m[z] ^= 909522486;
        ...
        finalize: function(f) {
            ...
            f = c.finalize(f);
            c.reset();
            return c.finalize(this._oKey.clone().concat(f))
        }
    })
})();
(function() {
    for (var b = CryptoJS, e = b.lib.BlockCipher, f = b.algo, c = [], h = [], y = [], l = [], B = [], a = [], m = [], z = [], u = [], w = [], g = [], p = 0; 256 > p; p++) g[p] = 128 > p ? p << 1 : p << 1 ^ 283;
    var d = 0,
        k = 0;
    for (p = 0; 256 > p; p++) {
        ...
        _doReset: function() {
            ...
            for (var J = this._keySchedule = [], H = 0; H < D; H++)
                if (H < C) J[H] = F[H];
                    ...
                    var E = J[H - 1];
                    H % C ? 6 < C && 4 == H % C && (E = c[E >>> 24] << 24 | c[E >>> 16 & 255] << 16 | c[E >>> 8 & 255] << 8 | c[E & 255]) : (E = E << 8 | E >>> 24, E = c[E >>> 24] << 24 | c[E >>> 16 & 255] << 16 | c[E >>> 8 & 255] << 8 | c[E & 255], E ^= O[H / C | 0] << 24);
                    J[H] = J[H - C] ^ E
                } F = this._invKeySchedule = [];
            for (C = 0; C < D; C++) H = D - C, E = C % 4 ? J[H] : J[H - 4], F[C] = 4 > C || 4 >= H ? E : m[c[E >>> 24]] ^ z[c[E >>> 16 & 255]] ^
                u[c[E >>> 8 & 255]] ^ w[c[E & 255]]
        },
        encryptBlock: function(D, F) {
            this._doCryptBlock(D, F, this._keySchedule, y, l, B, a, c)
        },
...
(function(b) {
    ...
    (function() {
        ...
        for (var z = 2, u = 0; 64 > u;) a(z) && (8 > u && (y[u] = m(b.pow(z, .5))), l[u] = m(b.pow(z, 1 / 3)), u++), z++
    })();
    var B = [];
    f = f.SHA256 = h.extend({
        _doReset: function() {
            this._hash = new c.init(y.slice(0))
        },
        ...
        _doFinalize: function() {
            var a = this._data,
                ...
                u = 8 * a.sigBytes;
            m[u >>> 5] |= 128 << 24 - u % 32;
            m[(u + 64 >>> 9 << 4) + 14] = b.floor(z / 4294967296);
            m[(u + 64 >>> 9 << 4) + 15] = z;
...
function calElmPos(b, e, f) {
    ...
    return {
        x1: c + b * Math.cos(Math.PI * y / e),
        y1: h + b * Math.sin(Math.PI * y / e),
        x2: c + b * Math.cos(Math.PI * f / e),
        y2: h + b * Math.sin(Math.PI * f / e)
    }
}
...
function spin(b, e) {
    if (!localStorage.getItem("remainTime")) {
        ...
        var c = 360 / _globalVars.config.totalSlices * f;
        ...
        _globalVars.elms.rotateGroup.animate(15E3, "circInOut").rotate(-3600 - c).afterAll(function() {
            setTimeout(function() {
                _globalVars.elms.pizza.children()[f - 1].animate(200).fill("#FFF").loop(15, !0).afterAll(function() {
...
function redeem(b, e) {
    ...
    document.querySelectorAll(".reward-list .items .item")[b].children[0].textContent = "Used";
    document.querySelectorAll(".reward-list .items .item")[b].children[0].classList.add("disabled");
    document.querySelectorAll(".reward-list .items .item")[b].children[0].removeAttribute("onclick");
    _globalVars.evt.rewardValue = f[b].price;
...
function saveReward(b) {
    ...
    b = JSON.parse(localStorage.getItem(cachedKey));
    b.unshift(e);
    localStorage.setItem(cachedKey, JSON.stringify(b));
    loadRewardBag()
}

function loadRewardBag() {
    if (localStorage.getItem(cachedKey)) {
        var b = JSON.parse(localStorage.getItem(cachedKey));
        for (var e = document.querySelector(".reward-list .items"), f = "", c = 0; c < b.length; c++) f = !0 === b[c].redeem ? f + ('<div class="item"><button class="btn-redeem disabled"><span>Used</span></button><div class="value" data-value="' + b[c].price + '">' + b[c].price + "</div></div>") : f + ('<div class="item disabled"><button class="btn-redeem" onclick="redeem(' + c + ')"><span>Redeem</span></button><div class="value" data-value="' +
            b[c].price + '">' + b[c].price + "</div></div>");
        e.innerHTML = f;
        document.querySelector(".burger-menu .counter").innerHTML = b.length
    }
}
...
function turnOffFilters() {
    ...
    for (var b = 0; b < _lightOuterMem.length; b++) _lightOuterMem[b].element.attr("filter", null);
    for (b = 0; b < _globalVars.elms.innerLightArr.length; b++) _globalVars.elms.innerLightArr[b].element.attr("filter", null);
    for (b = 0; b < _globalVars.elms.textArr.length; b++) _globalVars.elms.textArr[b].attr("filter", null);
...
function turnOnFilters() {
    ...
    if (2 <= _globalVars.config.graphicOption)
        for (var b = 0; b < _lightOuterMem.length; b++) _lightOuterMem[b].element.filter(setFilter(2, !0));
    if (2 <= _globalVars.config.graphicOption)
        for (b = 0; b < _globalVars.elms.innerLightArr.length; b++) _globalVars.elms.innerLightArr[b].element.filter(setFilter(1,
            !1));
    if (1 <= _globalVars.config.graphicOption)
        for (b = 0; b < _globalVars.elms.textArr.length; b++) _globalVars.elms.textArr[b].filter(setFilter(10, !1));
...
function setFilter(b, e) {
    var f = new SVG.Filter,
        c = f.offset(0, 0).gaussianBlur(b);
    f.blend(f.source, c);
...
function renderImageReward(b, e, f, c) {
    ...
    c -= _globalVars.config.sliceWidth;
    12 === y && (0 === e && (f += 0, c += 15, l = -90), 1 === e && (f -= 0, c += 15, l = -55), 2 === e && (f -= 0, c += 15, l = -35), 3 === e && (f -= 10, c += 15, l = 0), 4 === e && (f -= 10, c += 15, l = 35), 5 === e && (f -= 10, c += 15, l = 55), 6 === e && (f -= 10, c += 15, l = 90), 7 === e && (f -= 10, c += 15, l = 115), 8 === e && (f -= 10, c += 0, l = 140), 9 === e && (f -= 10, c += 0, l = 180), 10 === e && (f -= 10, c +=
        0, l = 215), 11 === e && (f -= 0, c += 15, l = 240));
    imgObj = b.image(h, 90);
    imgObj.move(f, c).rotate(l);
...
var _animation = {
        outerLight: {
            on: function() {
                for (var b, e = 0; e < _lightOuterMem.length; e++) b = .1 * e + "s", "3" === _globalVars.config.wheelUX ? _lightOuterMem[e].element.animate({
                    ease: "<",
                    delay: b
            ...
            off: function() {
                for (var b = 0; b < _lightOuterMem.length; b++) "3" === _globalVars.config.wheelUX && _lightOuterMem[b].element.finish().fill("#ffa000"), "2" === _globalVars.config.wheelUX && _lightOuterMem[b].element.finish().fill("#F4F4F4"),
                    "0" !== _globalVars.config.wheelUX && "1" !== _globalVars.config.wheelUX || _lightOuterMem[b].element.finish().fill("#FFFFB0")
            }
        }
    },
...
function drawGraphic() {
    ...
    if ("5" === _globalVars.config.wheelUX) {
        var f = b.gradient("linear", function(a) {
            a.at(0, "#56042C");
            a.at(1, "#56042C")
        });
        outerCircle = b.circle(_width - 30).move(15, 15);
        outerCircle.fill(f);
        f = b.circle("100%").attr({
            fill: "#d91173"
        });
        f.radius("45%");
        outerSpin = b.circle("100%").attr({
            fill: "#d2096b"
        });
        var c = b.gradient("radial", function(a) {
            a.at(0, "#d91173");
            a.at(1, "#56042C")
        })
    }
    "4" === _globalVars.config.wheelUX && (f = b.gradient("linear", function(a) {
        a.at(0,
            "#222222");
        a.at(1, "#333333")
    }), outerCircle = b.circle(_width - 30).move(15, 15), outerCircle.fill(f), f = b.circle("100%").attr({
        fill: "#E8DA9F"
    }), f.radius("45%"), outerSpin = b.circle("100%").attr({
        fill: "#23051D"
    }), c = b.gradient("radial", function(a) {
        a.at(0, "#FFFFFF");
        a.at(1, "#4E82CB")
    }));
    "3" === _globalVars.config.wheelUX && (f = b.gradient("linear", function(a) {
            a.at(0, "#ffa000");
            a.at(1, "#ffa000")
        }), outerCircle = b.circle(_width - 30).move(15, 15), outerCircle.fill(f), f = b.circle("100%").attr({
            fill: "#ffa000"
    ...
    "2" === _globalVars.config.wheelUX && (f = b.gradient("linear", function(a) {
        a.at(0, "#3E192A");
        a.at(1, "#2E0928")
    }), outerCircle = b.circle(_width - 30).move(15, 15), outerCircle.fill(f), f = b.circle("100%").attr({
        fill: "#666"
    ...
    "1" === _globalVars.config.wheelUX &&
        (f = b.gradient("linear", function(a) {
            a.at(0, "#E8DA9F");
            a.at(1, "#E8DA9F")
        }), outerCircle = b.circle(_width - 30).move(15, 15), outerCircle.fill(f), f = b.circle("100%").attr({
            fill: "#E8DA9F"
    ...
    "0" === _globalVars.config.wheelUX && (f = b.gradient("linear", function(a) {
            a.at(0, "#3E192A");
            a.at(1, "#2E0928")
        }), outerCircle = b.circle(_width - 30).move(15, 15), outerCircle.fill(f), f = b.circle("100%").attr({
            fill: "#666"
    ...
    c.from(.5, 0).to(.5, 0).radius(.4);
    outerSpin.radius("14%");
    outerSpin.fill(c);
    "4" === _globalVars.config.wheelUX ? (innerSpin = b.circle("100%").attr({
        fill: "#501245"
    }), e = b.gradient("linear", function(a) {
        a.at(0, "#FFFFFF");
        a.at(1, "#56042C")
    })) : (innerSpin = b.circle("100%").attr({
        fill: "#501245"
    }), e = b.gradient("linear", function(a) {
        a.at(0, "#23051D");
        a.at(1, "#521246")
    ...
    innerSpin.radius("10%");
    innerSpin.fill(e);
    spinLabel = b.text("SPIN");
    spinLabel.font({
        size: 0,
        fill: "#fff"
    });
    spinLabel.move("42%", "44%");
    spinImage = b.image(_globalVars.config.brandLogo, 160);
    spinImage.move(_centerX / 1.27, _centerY / 1.26);
    _lightOuterMem = [];
    if (12 >= parseInt(_globalVars.config.totalSlices) && "4" !== _globalVars.config.wheelUX && "5" !== _globalVars.config.wheelUX)
        for (var h = 0; h < 2 * _globalVars.config.totalSlices; h++) e = calElmPos(_width / 2.08, 2 * _globalVars.config.defaultStartDeg, h), e = "3" === _globalVars.config.wheelUX ?
            b.ellipse(30, 30).fill("#ffa").move(e.x1 - 15, e.y1 - 15).attr("id", h) : "2" === _globalVars.config.wheelUX ? b.ellipse(30, 30).fill("#F4F4F4").move(e.x1 - 15, e.y1 - 15).attr("id", h) : b.ellipse(30, 30).fill("#FFFFB0").move(e.x1 - 15, e.y1 - 15).attr("id", h), _lightOuterMem.push({
                element: e,
                filter: blur
            }), _globalVars.elms.outerLight.add(e);
    for (h = 0; h < _globalVars.config.totalSlices; h++) {
        c = "M" + _centerX + "," + _centerY + " ";
        var y = _width / 2.2;
        e = calElmPos(y, _globalVars.config.defaultStartDeg, h);
        e = b.path(c + "L" + e.x1 + "," + e.y1 + " A" + y + "," +
            y + " 0 0,1 " + e.x2 + "," + e.y2 + " z");
        var l = "";
        "5" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear", function(a) {
            a.at(0, "#d2096b");
            a.at(1, "#d91173")
        }) : b.gradient("radial", function(a) {
            a.at(0, "#FEC310");
            a.at(1, "#FEC310")
        }), e.fill(l), 5 === parseInt(_globalVars.config.totalSlices) && (l = b.gradient("linear", function(a) {
            a.at(0, "#fff");
            a.at(1, "#fff")
        }), e.fill(l)), e.stroke({
            color: "#56042C",
        ...
        "4" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear",
            function(a) {
                a.at(0, "#4E82CB");
                a.at(1, "#2D66C4")
            }) : b.gradient("radial", function(a) {
        ...
        "2" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear",
            function(a) {
                a.at(0, "#EA352F");
                a.at(1, "#222222")
            }) : b.gradient("radial", function(a) {
        ...
        "1" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear", function(a) {
            a.at(0, "#161313");
            a.at(1, "#222222")
        }) : b.gradient("radial", function(a) {
            a.at(0, "#E8DA9F");
        ...
        "0" === _globalVars.config.wheelUX && (l = 0 === h || 4 === h || 8 === h || 12 === h || 16 === h || 20 === h || 24 === h || 28 === h || 32 === h || 36 === h ? b.gradient("radial", function(a) {
            a.at(0, "#D80001");
            a.at(1, "#BC1505")
        }) : 1 === h || 5 === h || 9 === h || 13 === h || 17 === h || 21 === h || 25 === h || 29 === h || 33 === h ? b.gradient("linear", function(a) {
            a.at(0,
                "#04756F");
            a.at(1, "#045E5C")
        }) : 2 === h || 6 === h || 10 === h || 14 === h || 18 === h || 22 === h || 10 === h || 10 === h || 10 === h ? b.gradient("linear", function(a) {
            a.at(0, "#FF8B00");
            a.at(1, "#D37201")
        }) : 36 !== parseInt(_globalVars.config.totalSlices) || 27 !== h && 31 !== h && 34 !== h ? b.gradient("radial", function(a) {
            a.at(0, "#400B35");
            a.at(1, "#320B28")
        }) : b.gradient("radial", function(a) {
            a.at(0, "#333333");
        ...
        if ("0" === _globalVars.config.wheelUX && 12 >= parseInt(_globalVars.config.totalSlices))
            for (c = 0; 5 > c; c++) e = _width / 2.5 - 40 * c, e = calElmPos(e, _globalVars.config.defaultStartDeg, h), e = b.ellipse(16, 16).fill("#FFFFB0").move(e.x1 - 8, e.y1 - 8), e.attr("id", c), _globalVars.elms.innerLightArr.push({
                element: e,
                filter: blur
    ...
    for (h = 0; h < _globalVars.config.totalSlices; h++) {
        ...
        "5" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear", function(a) {
            a.at(0, "#222222");
            a.at(1, "#333333")
        }) : b.gradient("linear", function(a) {
            a.at(0, "#F1F1F1");
            a.at(1, "#F4F4F4")
        }), 5 === parseInt(_globalVars.config.totalSlices) && (l =
            b.gradient("linear", function(a) {
                a.at(0, "#EB3323");
                a.at(1, "#FF8B00")
            })));
        "4" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear", function(a) {
            a.at(0, "#222222");
            a.at(1, "#333333")
        }) : b.gradient("linear", function(a) {
            a.at(0, "#F1F1F1");
            a.at(1, "#F4F4F4")
        }), 5 === parseInt(_globalVars.config.totalSlices) && (l = b.gradient("linear", function(a) {
            a.at(0, "#EB3323");
            a.at(1, "#FF8B00")
        })));
        "3" === _globalVars.config.wheelUX && (0 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#f6546a");
                a.at(1, "#f6546a")
            })),
            1 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#2bfe72");
                a.at(1, "#2bfe72")
            })), 2 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#ffff00");
                a.at(1, "#ffff00")
            })), 3 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#ffffff");
                a.at(1, "#ffffff")
            })), 4 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#FFA500");
                a.at(1, "#FFA500")
            })), 5 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#CD853F");
                a.at(1, "#CD853F")
            })), 6 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#0080ff");
                a.at(1, "#0080ff")
            })), 7 === h && (l = b.gradient("linear",
                function(a) {
                    a.at(0, "#b2fbbb");
                    a.at(1, "#b2fbbb")
                })), 8 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#c9658f");
                a.at(1, "#c9658f")
            })), 9 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#666");
                a.at(1, "#ddd")
            })), 10 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#987dc5");
                a.at(1, "#987dc5")
            })), 11 === h && (l = b.gradient("linear", function(a) {
                a.at(0, "#e0eeee");
                a.at(1, "#838b8b")
            })));
        "2" === _globalVars.config.wheelUX && (l = 0 === h || 0 === h % 2 ? b.gradient("linear", function(a) {
            a.at(0, "#222222");
            a.at(1, "#333333")
        }) : b.gradient("linear",
            function(a) {
        ...
        "0" === _globalVars.config.wheelUX && (l = b.gradient("linear", function(a) {
            a.at(0, "#F1F1F1");
            a.at(1, "#F4F4F4")
        }));
        if (12 >= parseInt(_globalVars.config.totalSlices))
            if (-1 < _globalVars.jsonData[h].value.indexOf(" ")) {
                ...
                c = b.text(function(a) {
                    for (var m =
                            0; m < B.length; m++) a.tspan(B[m]).newLine(), a.font({
                        size: 24,
                        fill: l,
                        anchor: "middle"
                    })
            ...
            } else c = "3" === _globalVars.config.wheelUX ? b.text(function(a) {
        ...
        else c = b.text(function(a) {
            a.tspan(_globalVars.jsonData[h].value).font({
                size: 20,
                fill: l
            })
        });
        c.rotate(0).move(e.x1, e.y1);
        12 === parseInt(_globalVars.config.totalSlices) && (c.opacity(0), renderImageReward(b, h, e.x1, e.y1));
        ...
        _globalVars.elms.text.add(c);
        _globalVars.elms.textArr.push(c)
    }
    shelfTop = b.rect(130, 70);
    shelfTop.cx(_centerX);
    shelfTop.cy(0);
    shelfTop.radius(10);
    l = "3" === _globalVars.config.wheelUX ? b.gradient("linear", function(a) {
        a.at(0, "#fb3");
        a.at(.1, "#ffa000");
        a.at(.5, "#ffa000");
        a.at(.9, "#ffa000");
    ...
    "5" === _globalVars.config.wheelUX && shelfTop.opacity(0);
    "1" === _globalVars.config.wheelUX && shelfTop.opacity(0);
    shelfBot = b.path("M 20 0 L 180 0 L 200 80  L 260 110 L -60 110 L 0 80 Z");
    shelfBot.cx(_centerX);
    shelfBot.cy(_height + 20);
    l = b.gradient("linear", function(a) {
        a.at(0, "#BDBDBD");
        a.at(.25, "#E2E1DF");
        a.at(.5, "#C0C0C0");
        a.at(.8, "#5D5D5D");
    ...
    "5" === _globalVars.config.wheelUX && shelfBot.opacity(0);
    "3" === _globalVars.config.wheelUX && shelfBot.opacity(0);
    "1" === _globalVars.config.wheelUX && shelfBot.opacity(0);
    outerNeedle = b.polygon("0,0 100,0 50,100");
    outerNeedle.cx(_centerX);
    outerNeedle.cy(30);
    outerNeedle.size(100, 70);
    "5" === _globalVars.config.wheelUX && (l = b.gradient("radial", function(a) {
        a.at(0, "#d91173");
        a.at(1, "#56042C")
    }));
    "4" === _globalVars.config.wheelUX && (l = b.gradient("radial", function(a) {
        a.at(0, "#F0F0F0");
        a.at(1, "#A0A0A0")
    }));
    "3" === _globalVars.config.wheelUX && (l = b.gradient("radial", function(a) {
        a.at(0, "#c9658f");
        a.at(1, "#222222")
    }));
    "2" === _globalVars.config.wheelUX && (l = b.gradient("radial", function(a) {
        a.at(0, "#FFFFFF");
        a.at(1, "#222222")
    }));
    "1" === _globalVars.config.wheelUX &&
        (l = b.gradient("radial", function(a) {
            a.at(0, "#A0A0A0");
            a.at(1, "#E8DA9F")
        }));
    "0" === _globalVars.config.wheelUX && (l = b.gradient("radial", function(a) {
        a.at(0, "#F0F0F0");
        a.at(1, "#A0A0A0")
    }));
    outerNeedle.fill(l);
    ...
    innerNeedle = b.polygon("0,0 60,0 30,60");
    innerNeedle.cx(_centerX);
    innerNeedle.cy(10);
    innerNeedle.size(60, 50);
    "5" === _globalVars.config.wheelUX && innerNeedle.fill("#f4f4f4");
...
function decryptedAES(b) {
    ...
    return CryptoJS.AES.decrypt(b, e, {
        iv: f
...
function sendEmail() {
    if (!1 === _globalVars.isProcessing) {
        _globalVars.isProcessing = !0;
        var b = decryptedAES(_dynamicParams.configEmail.emailSendAddress),
            ...
            c = document.querySelector("#customer-email").value,
        ...
        Email.send({
            ...
            To: f,
            From: b,
            Subject: c + " You have a winner is waiting for your reward!",
            Body: c + " Win " + (h ? h[0].price : "$0")
        }).then(function(y) {
            _globalVars.isProcessing = !1;
